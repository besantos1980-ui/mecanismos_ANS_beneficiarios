...
def processar_sql():
    ...
    # Carrega planos
    con.execute(f"CREATE OR REPLACE VIEW planos_base AS SELECT * FROM read_csv_auto('{path_planos}', all_varchar=True) WHERE COBERTURA = 'Assistência Médica'")

    for ano in ANOS_INTERESSE:
        # AQUI O TRUQUE: O '**' diz ao DuckDB para procurar em todas as subpastas (estados)
        ...
        try:
            sql = f"""
                SELECT 
                    ...
                    SUM(CAST(d.QT_ITEM_EVENTO_INFORMADO AS DOUBLE)) as QTDE,
                    SUM(CAST(d.VL_ITEM_PAGO_FORNECEDOR AS DOUBLE)) as VALOR
